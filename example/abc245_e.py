...
choco = [(False, A[i], B[i]) for i in range(N)]
box = [(True, C[i], D[i]) for i in range(M)]
choco_box = sorted(choco + box, key=itemgetter(1, 0), reverse=True)
multiset = PseudoMultiset(B + D)

for is_box, _, width in choco_box:
    if is_box:
        multiset.add(width)
    else:
        min_box_ge_choco = multiset.min_ge(width)
        if min_box_ge_choco is None:
            print('No')
            exit()
        else:
            multiset.sub(min_box_ge_choco)
